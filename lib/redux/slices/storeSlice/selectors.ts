...
export const selectOpenStores = (state: ReduxState) => {
    return state.storeList.storeList.filter(store => store.openStatus === OpenStatus.OPEN);
};

// 取得指定 ID 的店鋪
export const selectStoreById = (state: ReduxState, id: string): StoreType | undefined => {
    return state.storeList.storeList.find(store => store.id === id);
};

// 取得每個店鋪中的產品數量
export const selectProductCountByStore = (state: ReduxState): Record<string, number> => {
...
export const selectStoreCountByStatus = (state: ReduxState): Record<string, number> => {
